            ...
            return (
              <section
                ...
                className={`island-shell rounded-2xl p-5 transition ${
                  item.featured ? "border-(--lagoon-deep)" : ""
                }`}
              >
                <div className="mb-3 flex items-start justify-between gap-2">
                  ...
                  {item.featured && (
                    <span className="inline-flex h-5 shrink-0 items-center rounded-full bg-(--lagoon-deep)/10 px-2 text-[11px] font-medium text-(--lagoon-deep)">
                      精选
                    </span>
                  )}
                </div>
                ...
                {tagList(item).length > 0 && (
                  <div className="mb-4 flex flex-wrap gap-1.5">
                    {tagList(item).map((tag) => (
                      <span
                        key={tag}
                        className="rounded-full bg-(--chip-bg) px-2 py-0.5 text-[11px] text-(--sea-ink-soft)"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                {isOpen && (
                  <div className="mb-4 max-h-80 overflow-y-auto rounded-lg border border-(--line) bg-(--surface) p-4">
                    <div
                      className="resume-editor-content text-sm leading-relaxed text-(--sea-ink)"
